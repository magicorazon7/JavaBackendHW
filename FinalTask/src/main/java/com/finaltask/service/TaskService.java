...
import com.finaltask.repository.TaskRepository;
...
public class TaskService {
    ...
    public List<Task> getTasksByGroup(User user, Long groupId) {
        ...
        return taskRepository.findByUserAndGroup(user, group);
    }
    ...
    public Task createTask(String title, String description, Long groupId, User user) {
        ...
        if (groupId != null) {
            TaskGroup group = taskGroupRepository.findById(groupId).orElseThrow(() ->
                    new RuntimeException("Group not found with id: " + groupId)); // кидаем ошибку если не находим группу по айди
            task.setGroup(group);
        }
    ...
    public Task updateTask(Long id, String title, String description, TaskStatus status, Long groupId, User user) {
        Task task = taskRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Task not found with id: " + id)); // при попытке обновить
        // несуществующую задачу

        if (!task.getUser().getId().equals(user.getId())) {
            throw new RuntimeException("You don't have permission to update this task"); // при попытке получить
            //задачу другого юзера
        }
    ...
    public List<Map<String, Object>> getTaskStatisticsByStatus(User user) {
        return taskRepository.getTaskStatisticsByStatus(user);
    }

    public List<Map<String, Object>> getTaskStatisticsByGroup(User user) {
        return taskRepository.getTaskStatisticsByGroup(user);
    }
